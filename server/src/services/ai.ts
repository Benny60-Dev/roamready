...
- DRIVE-TIME CONSTRAINT — HARD RULE: The user has a max daily drive time set in their travelProfile (\`maxDriveHours\`, in hours). Treat this as a HARD upper bound on each leg between consecutive stops, NOT an average across the trip.
  ...
  If a planned leg between two consecutive stops would exceed the user's limit, you MUST insert one or more OVERNIGHT_ONLY stops in plausible mid-route cities to break the leg up. Common transit-stop cities along major highways (e.g. Las Cruces, El Paso, Van Horn, Fort Stockton on I-10; Flagstaff, Albuquerque, Amarillo on I-40) are good candidates — pick whichever falls roughly within the user's daily limit from the previous stop.
