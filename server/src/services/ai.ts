...
export async function chatWithAI(
  ...
  const noHomeDirective = hasHomeOnFile
    ? ''
    : '\n\n⚠ NO HOME ADDRESS ON FILE — This user has no saved home city or address. When they have not named a starting city, you MUST ask "Where will you be starting from?" and wait for their answer before planning. Do NOT name, assume, invent, or borrow any starting city — not from these instructions\' examples, not from anywhere. If the user says "home", "my home", "from home", "use my home address", or any similar reference to home, do NOT treat that as a known location and do NOT invent or name a city. This includes when the user asks YOU to choose or supply the starting point — e.g. "you pick", "pick a home for me", "choose a starting point", "from my home address you pick", or any request that you select, guess, or invent the origin. Deferring the DESTINATION choice to you (a surprise trip) NEVER extends to the starting location: you may choose where they GO, but you must NEVER choose, invent, or assume where they START FROM. A named DESTINATION is NOT a starting location: if the user tells you where they want to GO (for example, a trip to some destination) and/or gives dates or a trip length but never names where they are STARTING FROM, you must STILL ask "Where will you be starting from?" first — a destination, dates, and duration do NOT give you enough to begin planning and do NOT authorize you to choose an origin. Until the user gives a real starting city, do NOT emit an <itinerary> block and do NOT populate the order-1 HOME stop with any city; the origin must be the user\'s own typed words, never a value you generate. Never output the sentence "I\'ll use your home address in [city]" or fill any home-address or [CITY] slot for this user — that line is reserved exclusively for users with a real saved home, which this user does not have. Tell them you don\'t have a home address on file and ask "Where will you be starting from?" before planning.'
...
- Starting location confirmation rules (must happen before any other trip questions):
  ...
  - If the user names a specific starting city, confirm it back as the very first response before asking anything else: "Got it — starting from [City, State]. Where are we headed?" (Use this line ONLY when the user actually named a starting city in THIS conversation — see the HARD RULE immediately below.)
  - HARD RULE — NEVER ASSERT AN ORIGIN YOU DO NOT HAVE: The two confirmation lines that NAME a starting city — "Got it — starting from [City, State]…" and "Perfect — I'll use your home address in [CITY]…" — are permitted ONLY when a REAL origin exists: EITHER the user typed a starting city or address in THIS conversation, OR a real home is on file (homeCity or homeLocation is present in the profile). When there is NO home on file AND the user has NOT typed a starting location, you are FORBIDDEN from emitting ANY sentence that names a starting city — including "starting from [City]", "I'll use your home address in [City]", "your home base in [City]", "leaving from [City]", "set off from [City]", or any equivalent phrasing. In that situation your ONLY permitted response about the origin is to ask, verbatim: "Where will you be starting from?" Never fill a [City]/[Origin] slot with a guessed, assumed, or invented city — if you do not have the origin from the user or the profile, ASK for it.
