...
/**
 * BUILD 3b — deterministic nights RECONCILER.
 *
 * Makes the built trip's total nights EXACTLY equal the user's captured target
 * (PlanningSession.partialTripData.requestedNights, from Build 3a) by adjusting
 * DESTINATION-stop nights only — padding up when short, trimming down when long.
 *
 * Invariants (all enforced below):
 *   - Reads the target from the linked session via the unique tripId relation —
 *     NO migration, NO new column.
 *   - Adjusts ONLY type === 'DESTINATION' stops. Never OVERNIGHT_ONLY, never
 *     HOME, and never the 0-night return-home closer (last stop, home city,
 *     nights 0) — that stop anchors the loop terminus.
 *   - Trim floor: a DESTINATION never drops below 1 night.
 *   - Distribution is PROPORTIONAL to each destination's current nights (the
 *     marquee proxy), with the remainder biased toward the bigger stops.
 *   - Edits Stop.nights ONLY — never adds, removes, or reorders stops.
 *   - recomputeStopDates remains the single source of truth for totals/dates:
 *     this function edits nights then calls it; it never stamps totals itself.
 *
 * Runs at BUILD time, AFTER expandLongLegs (so transit nights are counted) and
 * BEFORE generateItinerary. Fails soft: any error logs and leaves the trip
 * as-built — reconciliation must never block trip creation.
 */
export async function reconcileNights(req: AuthRequest, res: Response, next: NextFunction) {
  try {
    const trip = await prisma.trip.findFirst({
      where: { id: req.params.id, userId: req.user!.id },
      include: { stops: { orderBy: { order: 'asc' } } },
    ...
    // Target: requestedNights off the linked planning session (tripId @unique).
    // Absent / non-positive-integer → NO-OP, leave the trip exactly as built.
    const session = await prisma.planningSession.findUnique({
      where: { tripId: trip.id },
      select: { partialTripData: true },
    })
    const ptd = (session?.partialTripData as any) ?? null
    const target = ptd?.requestedNights
    if (!Number.isInteger(target) || target <= 0) {
      return res.json({ reconciled: false, reason: 'no-target' })
    }

    const stops = trip.stops as any[]
    // Canonical built-nights count — mirrors recomputeStopDates (OVERNIGHT_ONLY
    // is always 1 night; everything else uses its stored nights).
    const nightsOf = (s: any) => (s.type === 'OVERNIGHT_ONLY' ? 1 : (s.nights ?? 0))
    const built = stops.reduce((n, s) => n + nightsOf(s), 0)
    if (built === target) {
      return res.json({ reconciled: false, reason: 'already-exact', built, target })
    }

    // Identify the round-trip return-home closer (excluded from adjustment),
    // mirroring computeTripShape's anchor rule: home = first HOME stop (else
    // stops[0]); closer = a DISTINCT last stop at the home city with 0 nights.
    const norm = (v?: string | null) => (v ?? '').toLowerCase().trim()
    const homeStop = stops.find(s => s.type === 'HOME') ?? stops[0]
    const lastStop = stops[stops.length - 1]
    const closerId =
      lastStop && homeStop && lastStop !== homeStop &&
      lastStop.type === 'DESTINATION' && (lastStop.nights ?? 0) === 0 &&
      norm(lastStop.locationName) === norm(homeStop.locationName)
        ? lastStop.id
        : null

    // Adjustable set: real DESTINATION stops only, excluding the closer.
    const dests = stops.filter(s => s.type === 'DESTINATION' && s.id !== closerId)
    if (dests.length === 0) {
      return res.json({ reconciled: false, reason: 'no-destinations', built, target })
    }

    // Working copy of nights, indexed parallel to `dests`.
    const cur = dests.map(d => d.nights ?? 0)
    const FLOOR = 1
    ...
    if (delta > 0) {
      ...
      const wsum = cur.reduce((a, b) => a + b, 0)
      const ideal = cur.map((n, i) =>
        wsum > 0 ? (delta * n) / wsum : delta / dests.length,
      )
      const add = ideal.map(x => Math.floor(x))
      let rem = delta - add.reduce((a, b) => a + b, 0)
      const byRemainder = ideal
        .map((x, i) => ({ i, frac: x - Math.floor(x), nights: cur[i], order: dests[i].order }))
        .sort((a, b) => b.frac - a.frac || b.nights - a.nights || a.order - b.order)
      for (let k = 0; rem > 0; k++, rem--) add[byRemainder[k % byRemainder.length].i] += 1
      add.forEach((a, i) => { cur[i] += a })
    } else {
      // ── TRIM: remove min(|delta|, capacity) nights, never below FLOOR.
      // Proportional-by-nights ideal, capped at each stop's removable headroom,
      // then redistribute leftover to stops that still have headroom (largest
      // remaining nights first). If capacity < need, clamp — leave a residual
      // gap rather than drop a stop (no banner this build). ──
      const need = -delta
      const removable = cur.map(n => Math.max(0, n - FLOOR))
      const capacity = removable.reduce((a, b) => a + b, 0)
      const toRemove = Math.min(need, capacity)
      const wsum = cur.reduce((a, b) => a + b, 0)
      const removed = cur.map((n, i) =>
        Math.min(removable[i], Math.floor(wsum > 0 ? (toRemove * n) / wsum : toRemove / dests.length)),
      )
      let rem = toRemove - removed.reduce((a, b) => a + b, 0)
      while (rem > 0) {
        const cand = dests
          .map((d, i) => ({ i, headroom: removable[i] - removed[i], remaining: cur[i] - removed[i], order: d.order }))
          .filter(c => c.headroom > 0)
          .sort((a, b) => b.remaining - a.remaining || a.order - b.order)
        if (cand.length === 0) break // hit all floors — residual gap remains
        removed[cand[0].i] += 1
        rem -= 1
      }
      removed.forEach((r, i) => { cur[i] -= r })
    }

    // Persist only the changed nights, then let recomputeStopDates re-stamp
    // Trip.totalNights / endDate and every stop's arrival/departure date.
    const writes = dests
      .map((d, i) => ({ d, nights: cur[i] }))
      .filter(({ d, nights }) => nights !== (d.nights ?? 0))
      .map(({ d, nights }) => prisma.stop.update({ where: { id: d.id }, data: { nights } }))

    if (writes.length === 0) {
      return res.json({ reconciled: false, reason: 'no-change', built, target })
    }

    await prisma.$transaction(writes)
    await recomputeStopDates(trip.id)

    const reconciledTotal = cur.reduce((a, b) => a + b, 0) +
      stops.filter(s => s.type !== 'DESTINATION' || s.id === closerId).reduce((n, s) => n + nightsOf(s), 0)
    const exact = reconciledTotal === target
    console.log(
      '[reconcileNights] tripId=%s target=%d built=%d → reconciled=%d exact=%s adjusted=%d destination(s)',
      trip.id, target, built, reconciledTotal, exact, writes.length,
    )
    res.json({ reconciled: true, target, builtBefore: built, builtAfter: reconciledTotal, exact, adjusted: writes.length })
  } catch (err: any) {
    // Fail soft — a reconciliation failure must never block the build. Log and
    // return a non-error JSON so the awaited client call resolves cleanly.
    console.error('[reconcileNights] FAILED tripId=%s (trip left as-built): %s', req.params.id, err?.message ?? err)
