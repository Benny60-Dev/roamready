...
function buildLiveTripState(trip: any, rigs: any[] = []): string {
  ...
  return [
    ...
    '',
    'Change / replace a destination (e.g. "change my destination to X", "go to X instead of Y", "make the end Z"): emit two proposals — remove_stop for the destination being replaced, then add_stop for the new one — reusing the replaced stop\'s existing nights. Anchor add_stop with after_stop = the stop that preceded the replaced one (omit after_stop to append if it was the last stop). Do NOT ask for nights, dates, or origin — carry over what the trip already has.',
    '',
